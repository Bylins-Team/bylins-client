...
compose.desktop {
    application {
        ...
        jvmArgs += listOf(
            ...
            // Сборщик мусора с паузами меньше миллисекунды. На G1 при занятых
            // 78 МБ случались полные сборки по 45 мс -- это заметная остановка
            // отрисовки посреди боя (#perf показывал "Old Generation: 3 раза, 134 мс").
            // Клиенту мада важнее ровность кадров, чем пропускная способность.
            "-XX:+UseZGC",
            // Куча сразу нужного размера: растить её с умолчания -- лишние сборки
            // на ровном месте, а 256 МБ клиент занимает и так
            "-Xms256m",
            "-Xmx1g"
        )
